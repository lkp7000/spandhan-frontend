...
export async function apiGetPurchaseOrderInvoice<T>(
    params: number
) {
    return ApiService.fetchData<T>({
        url: `/purchaseorder/readpo/${params}`,
        method: 'get',
    })
}
export async function apiGetSalesProducts<T, U extends Record<string, unknown>>(
    ...
    return ApiService.fetchData<T>({
        url: '/purchaseorder/products',
        method: 'post',
        data,
    })
}

// export async function apiGetSalesProductList<T, U extends Record<string, unknown>>(
...
export async function apiSearchByPurchaseOrder<T>(
        params: string
    ) {
    return ApiService.fetchData<T>({
        url: `/purchaseorder/searchpo/${params}`,
        method: 'get'
    })
}
